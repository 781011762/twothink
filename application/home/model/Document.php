...
//     );
    ...
    public function prev($info){
        $map = array(
            ...
            'deadline'     => 'deadline = 0 OR deadline > ' . time(),
        );
    ...
    public function updates(){
        ...
        /* 添加或新增扩展内容 */
        $logic = logic($data['model_id']);//TODO
    ...
    public function part($id, $page = 1, $field = true, $logic = true){
        ...
        $info = $this->field($field)->with('picture')->where($map)->page($page, 10)->order('id')->select();
        if(!$info) {
            $this->error = '该文档没有段落！';
            return false;
        }
    ...
    protected function getStatus(){
        ...
        $check = model('Category')->getFieldById($cate, 'check');
        if($check){
            $status = 2;
        }else{
            $status = 1;

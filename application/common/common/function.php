...
function addons_url($url, $param = [])
{
    ...
    return url("home/addons/execute", $params);
}
/**
 * 获取插件类的类名
 * @param $name 插件名
...
function get_username($uid = 0){
    ...
    if(isset($list[$key])){ //已缓存，直接使用
        ...
        $User = new app\common\controller\UcApi();
        $info = $User->info($uid);
...
function get_model_attribute($model_id, $group = true,$fields=true){
    ...
    if(!isset($list[$model_id])){
        ...
        $extend = \think\Db::name('Model')->getFieldById($model_id,'extend');

        if($extend){
            $map = array('model_id'=> array("in", array($model_id, $extend)));
        }
        $info = \think\Db::name('Attribute')->where($map)->field($fields)->select();
        $list[$model_id] = $info;
    }
    $attr = array();
    if($group){
        ...
        $model     = \think\Db::name("Model")->field("field_sort,attribute_list,attribute_alias")->find($model_id);
...
function api($name,$vars=array()){
    ...
    $callback  = 'app\\'.$module.'\\controller\\'.$classname.'Api::'.$method;
    if(is_string($vars)) {
        parse_str($vars,$vars);
    }
...
function get_cover($cover_id, $field = null){
    ...
    $picture = \think\Db::name('Picture')->where(array('status'=>1))->getById($cover_id);
    if($field == 'path'){
